/**
 * Sentry — MUST be imported first in `src/index.ts`, before mqtt / better-sqlite3
 * / the HTTP server, so the SDK can patch them (hardening Phase 15).
 *
 * No DSN → `Sentry.init` is a no-op and the gateway runs exactly as before, so
 * this can land ahead of the Sentry project existing. Set `SENTRY_DSN` per
...
Sentry.init({
  ...
  release: process.env.SENTRY_RELEASE || `pet-iot-edge-gateway@${VERSION}`,
